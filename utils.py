...
def SliceSelection(num_slices, masks, d):
    # Selects specific adjacent slices based on distance 'd'
    i_slices =[] 
    if d != -1:
        ...
        if index - (2*d) <= 0:
            if index - d <= 0:
                i_slices.append(index + d)
                i_slices.append(index + (2*d))
                i_slices.append(index + (3*d))
            else:
                i_slices.append(index - d)
                i_slices.append(index + d)
                i_slices.append(index + (2*d))
        else:
            if index + (2*d) >= masks.shape[-1]-1:
                if index + d >= masks.shape[-1]-1:
                    i_slices.append(index - d)
                    i_slices.append(index - (2*d))
                    i_slices.append(index - (3*d))
                else:
        ...
        for i in range (num_slices):
            index = random.randint(5,masks.shape[-1]-5)
            i_slices.append(index)
    return i_slices

def save_csv(feat_name, loss):
    # Appends the loss results for a specific patient to a CSV file
    dataset = feat_name.split('-')[0] +  feat_name.split('-')[1]

    if os.path.exists('results_.csv'):
        data = pd.read_csv('results_.csv')
        new_row = pd.DataFrame({'dataset': [dataset], 'patient': [feat_name], 'loss': [loss]})
        data = pd.concat([data, new_row], ignore_index=True)
    else:
        data = pd.DataFrame({'dataset': [dataset], 'patient': [feat_name], 'loss': [loss]})

    data.to_csv('results_.csv', index=False)
...
def CalculateBoxes(masks, overlap):
    # Dynamically generates bounding boxes scaled by the defined overlap
    box_coords = []
    for i_slice in range(masks.shape[0]):
        box = find_box_from_mask(masks[i_slice, :, :].squeeze())
        ...
        scaling_factor = 100 / overlap
        new_width = box_width * scaling_factor
        new_height = box_height * scaling_factor
        
        # Dimensional correction applied to the boundaries
        x_min_adj = max(0, box_center_x - new_width / 2)
        x_max_adj = min(masks.shape[-1], box_center_x + new_width / 2)
        y_min_adj = max(0, box_center_y - new_height / 2)
        y_max_adj = min(masks.shape[-2], box_center_y + new_height / 2)
        
        adjusted_box = [x_min_adj, y_min_adj, x_max_adj, y_max_adj]
        box_tensor = torch.tensor(adjusted_box, dtype=torch.float32)
        
        box_coords.append(box_tensor)
    
    box_coords = torch.stack(box_coords)
    return box_coords


def find_box_from_mask(masks):
    # Extracts the extreme coordinates of the tumor mask
    y, x = np.where(masks == 1)
    if len(x)==0: # There is no mask 
        x0 = 0
        x1 = 0
        y0 = 0
        y1 = 0
...
def view_preds(mask_slice, pred_slice, t2f_slice, feat_name, pt_slice, box_slice, loss, training = True):

    from skimage.measure import find_contours

    # print(mask_slice.shape, pred_slice.shape, t2f_slice.shape, feat_name, pt_slice)
    pred_slice = torch.sigmoid(pred_slice).detach().cpu().numpy()
    ...
    if training==True:
        if args.box=='True':
            x_min, y_min, x_max, y_max = box_slice
            width = x_max - x_min
            height = y_max - y_min

            rect = patches.Rectangle((x_min, y_min), width, height, linewidth=2, edgecolor='hotpink', facecolor='none')

            fig, axes = plt.subplots(nrows=1, ncols=4, figsize=(11, 11))
            axes[0].imshow(t2f_slice, cmap = 'gray')
            axes[0].set_title('T2-FLAIR')
            axes[1].imshow(mask_slice, cmap = 'gray')
            axes[1].add_patch(rect) 
            axes[1].set_title('mask with prompt')
            axes[2].imshow(pred_slice, cmap = 'gray')
            axes[2].set_title('sigmoid pred - loss: {0:.{1}f}'.format(loss, decimals))
            axes[3].imshow(pred_bin, cmap = 'gray')
            axes[3].set_title('binary pred') 
            plt.savefig('predictions/'+ feat_name+'.png', bbox_inches='tight', pad_inches=0)
            plt.close()
            ...
            axes[0].set_title('T2-FLAIR')
            axes[1].imshow(mask_slice, cmap = 'gray')
            axes[1].plot(pt_slice[1], pt_slice[0], marker='*', color="hotpink") 
            axes[1].set_title('mask with prompt')
            axes[2].imshow(pred_slice, cmap = 'gray')
            axes[2].set_title('sigmoid pred - loss: {0:.{1}f}'.format(loss, decimals))
            axes[3].imshow(pred_bin, cmap = 'gray')
        ...
        altura, anchura = t2f_slice.shape

        mask_contours = find_contours(mask_slice, level=0.5)
        pred_contours = find_contours(pred_bin, level=0.5)

        fig, axes = plt.subplots(nrows=1, ncols=1, figsize=(11, 11))

        axes.imshow(t2f_slice, cmap='gray')

        patches = []  

        for contorno in mask_contours:
            contorno_ajustado = contorno[:, [1, 0]]
            polygon = Polygon(contorno_ajustado, closed=True)
            patches.append(polygon)

        p = PatchCollection(patches, facecolor='hotpink', edgecolor='none', alpha=0.5)
        axes.add_collection(p)

        patches = []  

        for contorno in pred_contours:
            contorno_ajustado = contorno[:, [1, 0]]
            polygon = Polygon(contorno_ajustado, closed=True)
            patches.append(polygon)

        p = PatchCollection(patches, facecolor='lightskyblue', edgecolor='none', alpha=0.5)
        # axes.add_collection(p)

        plt.savefig('figs/gt/ssa/' + feat_name + '.png', bbox_inches='tight', pad_inches=0) 
        plt.close()
    return

def save_preds(pred_slice, name):

    print(torch.max(pred_slice), torch.min(pred_slice))
    pred_slice = torch.sigmoid(pred_slice).cpu().numpy()
    # pred_slice = pred_slice.cpu().numpy()
    ...
    plt.savefig('predictions/'+ name+'.png', bbox_inches='tight', pad_inches=0)
...
def render_vis(
    ...
    # Prepare directories and structures
    now = datetime.now()
    ...
    try:
        for i in tqdm(range(1, max(thresholds) + 1), disable=(not progress)):
            ...
            if args.disc:
                ...
                # End of domain loss calculation
...
def eval_seg(pred,true_mask_p,threshold):
    ...
    if c == 2:
        ...
        for th in threshold:
            ...
            # IOU metric calculated for numpy
            iou_d += iou(disc_pred,disc_mask)
            iou_c += iou(cup_pred,cup_mask)

            # DICE metric calculated for torch
