...
from .modeling import ImageEncoderViT, ImageEncoderViT_4_chan, MaskDecoder, PromptEncoder, Sam
...
def _build_sam(
    ...
    if args.four_chan==True:
        sam = Sam(
        args,
        image_encoder=ImageEncoderViT_4_chan(
            args = args,
            depth=encoder_depth,
    ...
    if checkpoint is not None:
        ...
        if 'state_dict' in torch.load(checkpoint).keys(): # generated weights 
        ...
        for name, param in sam.named_parameters():
            if name in state_dict.keys() and state_dict[name].shape != param.shape:
                print('NOT BEING LOADED: ', name,'; Checkpoint shape: ', state_dict[name].shape, '; Net shape: ', param.shape)
                del state_dict[name]
